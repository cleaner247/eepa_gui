...
LIBS_SEARCH_PATH =/usr/local/lib
...
$(EXE):$(OBJS)
	$(CXX) -o $@ $(filter %.o, $^) $(LIBS_SEARCH_PATH) $(LIBS) $(LDSTATIC)

$(DIR_DEPS)/%.dep: %.cpp
	@echo "Making $@ ..."
	@set -e;\
		$(CXX) $(INCLUDES) -E -MM $(filter %.cpp, $^) | sed 's,\(.*\)\.o[ :]*,$(DIR_OBJS)/\1.o $@:,g' > $@

$(DIR_OBJS)/%.o: %.cpp
	$(CXX) $(INCLUDES) $(CXXFLAGS) -o $@ -c $(filter %.cpp, $^)
