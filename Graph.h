...
struct Arc : Shape {
  Arc(Point p, int w, int h, int a_start = 0,
      int a_end = 360)  // center, min, max distance from center, start angle,
                        // end angle
      : w(w), h(h), start(a_start), end(a_end) {
    add(Point(p.x - w, p.y - h));
  }
  ...
  void set_minor(int hh) { h = hh; }
  int minor() const { return h; }
  void set_angle_start(int angle) { start = angle; }
  int angle_start() const { return start; }
  void set_angle_end(int angle) { end = angle; }
  int angle_end() const { return end; }
