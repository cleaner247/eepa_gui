...
struct ButtonWithPtr : Widget {
  ButtonWithPtr(Point xy, int w, int h, const string& label, Callback cb,
                void* data)
      : Widget(xy, w, h, label, cb), data_ptr_(data) {}
...
struct Menu : Widget {
  ...
  {
    for (unsigned int i = 0; i < selection.size(); ++i) selection[i].show();
  }
  void hide()  // hide all buttons
  {
    for (unsigned int i = 0; i < selection.size(); ++i) selection[i].hide();
  }
